...
// Initialize particle system when the document is ready
document.addEventListener('DOMContentLoaded', () => {
    window.typeParticleSystem = new TypeParticleSystem();
    
    // Extend existing functions to add particle effects
    const originalCreatePerfectWordEffect = window.createPerfectWordEffect;
    if (originalCreatePerfectWordEffect) {
        window.createPerfectWordEffect = function(word) {
            originalCreatePerfectWordEffect(word);
    ...
    const originalShowStreakCounter = window.showStreakCounter;
    if (originalShowStreakCounter) {
        window.showStreakCounter = function(streak) {
            originalShowStreakCounter(streak);
    ...
    // Add particle effect to falling words
    const originalCreateFallingWord = window.createFallingWord;
    if (originalCreateFallingWord) {
        window.createFallingWord = function(word) {
            originalCreateFallingWord(word);
            ...
            if (window.typeParticleSystem) {
                window.typeParticleSystem.createParticleEffect(x, y, 'correct', 5);
            }
        };
    }
});
